...
db.any(
  'create table if not exists consumer(id serial primary key, user_id varchar(200),timestamp timestamp,age int,weight int, height int,gender varchar(10),weightgoal varchar(100),consume int,activity varchar(100),exercise int);'
)
...
server.get('/insert', (req, res) => {
  txt = [];
  db.any(
    "insert into consumer(user_id,timestamp,age,gender,weight,height,weightgoal,activity) values('test',now(),'44','female','64','164','Stay the same weight','a')"
  )
    .then(data => {
      res.json(data);
    })
...
server.get('/getreq', (req, res) => {
  ...
  db.any(
    'select * from consumer where user_id=$1 AND weight>0 AND height>0 AND age>0',
    [user_id]
  )
    .then(data =>
      res.json(
...
server.get('/getdaily', (req, res) => {
  ...
  db.any(
    'select sum(consume) as cons,sum(exercise) as exc from consumer where user_id=$1 and date(timestamp)=date(now())',
    [user_id]
  )
    .then(data => {
      rem =
        parseFloat(req) -
        parseFloat(data[0]['cons']) +
        parseFloat(data[0]['exc']);
      res.json({
        speech:
          'Your daily calorie ' +
          req +
    ...
    .then(function(results) {
      ...
      fatAPI
        ...
        .then(function(result) {
          ...
          result_data = JSON.stringify(
            result.exercise.minutes.minutes[0].calories
          );
          res.send(
            'Result: ' +
              exercise +
              ...
              'kcal' +
              '. Minutes: 1 ' +
              result.exercise.minutes.minutes[0].minutes
          ); // Send response to user
        });
    });
});
...
server.post('/', (req, res) => {
  ...
  if (req.body.result.action == 'get-weight') {
    ...
    db.any(
      'insert into consumer(user_id,timestamp,weight) values($1,now(),$2)',
      [user_id, req.body.result.resolvedQuery.split(' ')[0]]
    )
  ...
  if (req.body.result.action == 'get-gender') {
    ...
    db.any('update consumer set gender=$1 where user_id=$2', [
      req.body.result.resolvedQuery.toLowerCase(),
      user_id
    ])
      .then(res => console.log(res))
      .catch(err => console.log(err));
  ...
  if (req.body.result.action == 'get-height') {
    ...
    db.any('update consumer set height=$1 where user_id=$2', [
      req.body.result.resolvedQuery.split(' ')[0],
      user_id
    ])
      .then(res => console.log(res))
      .catch(err => console.log(err));
  ...
  if (req.body.result.action == 'get-age') {
    ...
    db.any('update consumer set age=$1 where user_id=$2', [
      req.body.result.resolvedQuery.split(' ')[0],
      user_id
    ])
      .then(res => console.log(res))
  ...
  if (req.body.result.action == 'get-activity') {
    ...
    db.any('update consumer set activity=$1 where user_id=$2', [
      req.body.result.resolvedQuery.toLowerCase(),
      user_id
    ])
      .then(res => console.log(res))
  ...
  if (req.body.result.action == 'get-weightgoal') {
    db.any('update consumer set weightgoal=$1 where user_id=$2', [
      ...
      .catch(err => console.log(err));
    db.any(
      'select * from consumer where user_id=$1 order by timestamp desc limit 1',
      [user_id]
    )
      .then(data => {
        rsp = {
          speech:
            'In order to reach your weight goal, you would have to consume ' +
            getRequiredCalorie(
              data[0]['weight'],
              data[0]['gender'],
              data[0]['height'],
            ...
            ' kcal a day, Do you want to start your diet today?',
          displayText:
            'In order to reach your weight goal, you would have to consume ' +
            getRequiredCalorie(
              data[0]['weight'],
          ...
          .then(function(result) {
            ...
            rsp = {
              speech:
                ...
                ' g |' +
                ' Fat: ' +
                result.food.servings.serving[0].fat +
                ' g. Is this kind of food correct? (Yes/No)',
              displayText:
                result_name +
                '. Per 1 ' +
                result.food.servings.serving[0].measurement_description +
  ...
  if (
    ...
    'slimmeget-food.slimmeget-food-yes.slimmeget-food-yes-custom'
  ) {
    var food = req.body.result.resolvedQuery;
    rsp = {
      speech: food,
    ...
    rapid
      ...
      .on('success', payload => {
        rsp = {
          speech:
            'Yay! You just burn ' +
            payload[0].exercises[0].nf_calories +
            ' k-calories. Keep it up!',
          displayText:
            'Yay! You just burn ' +
            payload[0].exercises[0].nf_calories +
            ' k-calories. Keep it up!',
        ...
        db.any(
          'insert into consumer(user_id,timestamp,exercise) values($1,now(),$2)',
          [user_id, payload[0].exercises[0].nf_calories]
        )
